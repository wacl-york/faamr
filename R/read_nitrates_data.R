...
#'
#' Reads and cleans core nitrates FAAM file. TODO update to use tidync and remove reshape dependency
#'
#'
#' @name read_nitrates_data
#' @param nitrates_file_list List containing file names
#' @param frequency Frequency in Hz to output the data at. FAAM nitrate files can be outputted at 1 or 10 Hz.
#'
#' @return Tibble
#' 
#' @author Shona Wilde
...
read_nitrates_data <- function(nitrates_file_list, frequency = 1) {
  ...
  purrr::map_dfr(nitrates_file_list, frequency = frequency,
                 .f = function(file, frequency){
                   ...
                   list_nc <- variables |> 
                     ...
                     purrr::map(~ncdf4::ncvar_get(nc, varid = .x))
                   ...
                   change_colname_value <- function(df, new_colname){
                     df |> dplyr::rename(!!new_colname := .data$value)
                   }
                   ...
                   data_1hz <- nc_array |> 
                     purrr::map(tibble::as_tibble) |> 
                     purrr::map(dplyr::mutate, date = .data$date) |> 
                     purrr::map2(names_array, change_colname_value) |> 
                     purrr::reduce(dplyr::left_join, "date") 
                   ...
                   if(frequency == 10)
                     ...
                     dplyr::select(date, flight_no, tidyselect::everything())
